...
export class VisualizerRenderer {
  ...
  constructor(private canvas: HTMLCanvasElement) {
    ...
    if (!ctx) throw new Error("Canvas 2D rendering is not available in this browser.");
    this.ctx = ctx;
    this.seedParticles();
  }
  ...
  drawFrame(settings: VisualizerSettings, metrics: AudioMetrics, assets: RenderAssets, time: number) {
    ...
    if (settings.style === "circular") this.drawCircularSpectrum(settings, metrics, minSide);
  ...
  private drawBackground(settings: VisualizerSettings, background: HTMLImageElement | undefined, time: number, pulse: number) {
    const ctx = this.ctx;
    const width = this.canvas.width;
    const height = this.canvas.height;
    ctx.save();
  ...
  private drawLogo(settings: VisualizerSettings, logo: HTMLImageElement | undefined, minSide: number, pulse: number) {
    const ctx = this.ctx;
  ...
  private drawCircularSpectrum(settings: VisualizerSettings, metrics: AudioMetrics, minSide: number) {
    const ctx = this.ctx;
  ...
  private drawWaveformRing(settings: VisualizerSettings, metrics: AudioMetrics, minSide: number, alpha = 0.82) {
    const ctx = this.ctx;
  ...
  private drawHorizontalBars(settings: VisualizerSettings, metrics: AudioMetrics, width: number, minSide: number) {
    const ctx = this.ctx;
    const bins = metrics.frequencyData;
    const bars = 96;
  ...
  private drawParticlePulse(settings: VisualizerSettings, metrics: AudioMetrics, minSide: number, time: number) {
    const ctx = this.ctx;
  ...
  private drawOuterGlow(settings: VisualizerSettings, metrics: AudioMetrics, minSide: number) {
    const ctx = this.ctx;
  ...
  private drawParticles(settings: VisualizerSettings, metrics: AudioMetrics, time: number, cx: number, cy: number, minSide: number) {
    const ctx = this.ctx;
    ctx.save();
    ctx.translate(cx, cy);
    ctx.globalCompositeOperation = "lighter";
